...
<input class="form-control me-2" name="q" type="search" placeholder="Typin'" aria-label="Typin'" value="${search_text}" required>
...
function file_code(name, encoded_name, size, bytes, url, ext, file_id, cookie_folder_id) {
    ...
    if (bytes <= 1024 * 1024 * 2) {
        ...
        $("#code_spinner").html("");
        $('#editor').html(`<div class="${UI.file_view_alert_class}" id="file_details" role="alert">Unfortunately, the file size is too large to be previewed. For your reference, the maximum file size allowed for previews is 2 MB.</div>`);
    }
}
...
function file_audio(name, encoded_name, size, url, file_id, cookie_folder_id) {
    ...
    <div class="container text-center"><br>
      ...
      <div class="card text-center">
        <div class="text-center">
          <div class="${UI.file_view_alert_class}" id="file_details" role="alert" font-family: monospace;">${name}<br>${size}</div>
          ${UI.disable_player ? `` : `
